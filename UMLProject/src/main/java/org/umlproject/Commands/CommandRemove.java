        ...
        switch (args[0]) {
            ...
            case "relationship" -> {
                ...
                //Case when user wishes to pick a relationship from a list to remove.
                    ...
                    if (selectedRelationship == -2) {
                        System.out.println("Exited from menu.");
                        return;
                    } else if (selectedRelationship == -1) {
                        System.out.println("No relationships to list!");
                        return;
                    }
            ...
            case "method" -> {
                ...
                //Case where methods are listed and user specifies index to remove.
                    ...
                    if (selectedMethod == -1) {
                        System.out.println("No Methods to list!");
                        return;
                    }
            ...
            case "param" -> {
                //Need 5 arguments exactly to remove a specific parameter.
                ...
                //------------------------------------------------------------------------------------------------------
                //if args equal 3, then the user needs to be presented with a list of overloaded methods to choose from.
                //After method is selected, print the parameter list of the method to the screen and allow the user
                //to select a parameter from the list to remove.
                else{
                    String className = args[1];
                    String methodName = args[2];
                    //Ensure that the class containing the desired method exists
                    ...
                    //Check if methods corresponding to the given method name argument exist before
                    //prompting user with a selection of a method.
                    ArrayList<UMLMethod> methodList = umlClass.getMethodsAll().get(methodName);
                    if (methodList == null || methodList.isEmpty()) {
                        System.out.println("No method exists!");
                        return;
                    ...
                    //retrieve parameter array list and convert into an array of UMLParameters to pass to
                    //promptUserSelectionFromList:
                    ArrayList<UMLParameter> parameters = selectedMethod.getParameters();
                    UMLParameter[] parameterArray = parameters.toArray(UMLParameter[]::new);
                    if(parameters == null){
                        System.out.println("Method has no Parameters!");
                        return;
                    }
                    UMLParameter selectedParameter = promptUserSelectionFromList(parameterArray);
                    if(selectedParameter == null){
                        System.out.println("No such parameter to remove!");
                        return;
                    }
                    //Remove parameter and print according to success.
                    boolean isSuccessful = selectedMethod.removeParameter(selectedParameter);
                    if (isSuccessful) {
                        System.out.println("Parameter successfully removed!");
                        return;
                    } else {
